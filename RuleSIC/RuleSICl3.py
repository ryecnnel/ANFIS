...
class DeepSIC(tf.keras.layers.Layer):
    def __init__(self, input_size, part_num):
        super(DeepSIC, self).__init__()
        self.n = input_size
        self.p = part_num
        ...
        self.a3 = tf.Variable(a.reshape(self.n+1, self.p), trainable=True)
        self.b3 = tf.Variable(b.reshape(self.n+1, self.p), trainable=True)
        ...
        self.y3 = tf.Variable(np.random.normal(0.5, 0.3, (self.n+1, self.p)), trainable=True)
    ...
    def second_layer(self, X):
        ...
        # 2層目出力
        y2 = tf.math.reduce_sum(tf.math.reduce_sum(h2 * self.y2) / tf.math.reduce_sum(h2))
        return y2
    
    def third_layer(self, X):
        # 3層目入力作成
        y2 = self.second_layer(X)
        X_ = tf.concat([X, [y2]], axis=0)
        x = tf.reshape(X_, [self.n+1, 1])
        # 3層目ルール群適合度
        h3 = tf.math.exp(-(self.a3 - x)**2 / (2*self.b3**2))
        # 3層目出力
        y3 = tf.math.reduce_sum(tf.math.reduce_sum(h3 * self.y3) / tf.math.reduce_sum(h3))
        return y3
    
    def loss(self, X, T):
        predicted_y = self.third_layer(X)
