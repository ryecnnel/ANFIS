...
class DeepSIRMs(tf.keras.layers.Layer):
    def __init__(self, input_size, part_num):
        ...
        self.w3 = tf.Variable(np.random.normal(0.5, 0.3, (self.n+1)), trainable=True)
    ...
    def third_layer(self, X):
        ...
        # 3層目出力
        y3 = tf.math.reduce_sum((tf.math.reduce_sum(h3 * self.y3, axis=1) / tf.math.reduce_sum(h3, axis=1)* self.w3))
        return y3
